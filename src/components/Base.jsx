...
function Base() {
    ...
    return (
        <Grid item xs={9}>
            ...
            <form className={classes.dynamicComponentsWrapper} noValidate autoComplete="off">
            {stepLevel()}
            </form>
                <Divider light className={classes.divider} />
                <Grid className={classes.bottomWrapper} container>
                    <Button disabled={flowState.step === 0} onClick={() => flowDispatch({type:constants.PREV_STEP}) }>Back</Button>
                    <Button variant="contained" color="primary" onClick={() => flowDispatch({type:constants.NEXT_STEP}) }>
